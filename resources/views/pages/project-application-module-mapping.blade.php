<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ $title ?? 'Project–Application–Module Mapping' }}</h2>
    </x-slot>
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                ...
                <div class="border-b border-slate-200 bg-slate-50 px-5 py-5">
                    <div class="grid gap-4 md:grid-cols-[1fr_auto_auto] md:items-center">
                        <div>
                            <p class="text-sm text-slate-600">{{ $description ?? 'Manage mapping of projects to applications and modules.' }}</p>
                        </div>
                        ...
                        <div class="flex justify-end">
                            <button type="button" onclick="openProjectApplicationModuleModal()" class="inline-flex items-center justify-center rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700">Add Mapping</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="max-h-[420px] overflow-auto">
                        <table class="min-w-full divide-y divide-slate-200">
                            ...
                            <tr>
                                ...
                                <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-purple-900">Module</th>
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                            @forelse($mappings as $mapping)
                                <tr>
                                    ...
                                    <td class="px-5 py-3 text-sm font-semibold text-slate-900">{{ $mapping->project_name }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-5 py-6 text-center text-sm text-slate-500">No project application module mappings found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div id="project-application-module-modal" class="fixed inset-0 z-50 hidden bg-slate-900/60 px-4 py-8">
        <div class="mx-auto max-w-2xl rounded-3xl bg-white shadow-2xl">
            <form id="project-application-module-form" method="POST" action="">
                @csrf
                <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                    <div>
                        <h3 id="project-application-module-modal-title" class="text-lg font-semibold text-slate-900">Add Mapping</h3>
                        <p class="text-sm text-slate-600">Create or update a project-application-module mapping.</p>
                    </div>
                    <button type="button" onclick="closeProjectApplicationModuleModal()" class="rounded-full p-2 bg-slate-100 text-slate-700 hover:bg-slate-200 hover:text-slate-900">
                ...
                <div class="space-y-4 px-5 py-5">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <select id="project_id" name="project_id" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-slate-400">
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">Modules</label>
                        <div id="module-select" class="relative">
                            <div class="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm transition focus-within:border-slate-400 focus-within:ring-2 focus-within:ring-emerald-100" data-multi-select="modules">
                                <div class="flex flex-wrap gap-2" data-multi-select-chips></div>
                                <input type="text" class="min-w-[140px] flex-1 bg-transparent text-sm text-slate-900 placeholder:text-slate-400 outline-none" placeholder="Search modules" data-multi-select-input autocomplete="off" />
                            </div>
                            <div class="absolute left-0 right-0 z-50 mt-1 hidden max-h-60 overflow-auto rounded-xl border border-slate-200 bg-white shadow-xl" data-multi-select-list></div>
                            <div data-multi-select-hidden class="hidden"></div>
                        </div>
                    ...
                    <div>
                        <p class="text-sm text-slate-500">Use this screen to map a project to one or more applications and modules.</p>
                    </div>

                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-4">
                        <button type="button" onclick="closeProjectApplicationModuleModal()" class="rounded-xl border border-slate-300 bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200">Cancel</button>
                        <button type="submit" class="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700" id="project-application-module-modal-submit">Save</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script>
        const optionsEndpoint = '{{ route('project.application.module.mapping.options') }}';
        const formState = {
            applications: [],
            modules: [],
        };
        ...
        function formatOptionLabel(item) {
            return item.application_name || item.module_name || item.project_name || '';
        }

        function getOptionById(list, id) {
            return list.find((item) => String(item.application_id) === String(id) || String(item.module_id) === String(id));
        }
        ...
        function initMultiSelect(name) {
            ...
            function buildOptionButton(option) {
                ...
                button.dataset.value = option.application_id || option.module_id;
            ...
            function renderList(filter = '') {
                ...
                const filtered = options.filter((option) => {
                    ...
                    const alreadySelected = selected.includes(String(option.application_id || option.module_id));
            ...
            function renderChips() {
                ...
                selected.forEach((value) => {
                    ...
                    hidden.type = 'hidden';
                    hidden.name = name === 'applications' ? 'application_ids[]' : 'module_ids[]';
                    hidden.value = value;
        ...
        const moduleSelect = initMultiSelect('modules');

        async function loadRealTimeOptions() {
            const response = await fetch(optionsEndpoint, { headers: { 'Accept': 'application/json' } });
            if (!response.ok) {
                console.error('Unable to load application/module options', response.statusText);
                return;
            }

            const data = await response.json();
            applicationSelect.setOptions(data.applications || []);
            moduleSelect.setOptions(data.modules || []);
        }

        async function openProjectApplicationModuleModal() {
            await loadRealTimeOptions();
            document.getElementById('project-application-module-modal-title').textContent = 'Add Mapping';
            document.getElementById('project-application-module-form').action = '{{ route('project.application.module.mapping.store') }}';
            ...
            moduleSelect.setItems([]);
            document.getElementById('project-application-module-modal-submit').textContent = 'Save';
            document.getElementById('project-application-module-modal').classList.remove('hidden');
        }

        function closeProjectApplicationModuleModal() {
            document.getElementById('project-application-module-modal').classList.add('hidden');
        }
        ...
        async function editProjectApplicationModule(data) {
            await loadRealTimeOptions();
            document.getElementById('project-application-module-modal-title').textContent = 'Edit Mapping';
            document.getElementById('project-application-module-form').action = '{{ url('/project-application-module') }}' + '/' + (data.mappingId || '');
            ...
            const applicationValues = String(data.applicationId || '').split(',').filter(Boolean);
            const moduleValues = String(data.moduleId || '').split(',').filter(Boolean);

            applicationSelect.setItems(applicationValues);
            moduleSelect.setItems(moduleValues);
            document.getElementById('project-application-module-modal-submit').textContent = 'Update';
            document.getElementById('project-application-module-modal').classList.remove('hidden');
        }
