<x-app-layout>
                            ...
                            <tbody id="mail-configuration-table-body" class="divide-y divide-slate-200 bg-white">
                                ...
                                <tr>
                                    <td class="px-5 py-4 text-sm font-semibold text-slate-900">
                                        {{ $configuration->state_name ?? 'All States' }}</td>
                                    <td class="px-5 py-4 text-sm text-slate-600">
                                        {{ implode(', ', json_decode($configuration->to_emails, true) ?: []) }}</td>
                                    <td class="px-5 py-4 text-sm text-slate-600">
                                        {{ implode(', ', json_decode($configuration->cc_emails, true) ?: []) ?: '-' }}
                                    </td>
                                    <td class="px-5 py-4 text-sm">
                                        <span
                                            class="rounded-full {{ $configuration->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }} px-2.5 py-1 text-xs font-semibold">
                                    ...
                                    <td class="px-5 py-4 text-sm text-slate-700">
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            <button type="button"
                                                class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">Edit</button>
                                            @endif
                                            @if($configuration->is_active && data_get($permissions, 'deactivate'))
                                            <button type="button"
                                                class="rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700 hover:bg-rose-100">Deactivate</button>
                                            @elseif(!$configuration->is_active && data_get($permissions, 'activate'))
                                            <button type="button"
                                                class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 hover:bg-emerald-100">Activate</button>
                                            @endif
                                            @if(data_get($permissions, 'delete'))
                                            <button type="button"
                                                class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">Delete</button>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                                @empty
                                <tr class="empty-row">
                                    <td colspan="5" class="px-5 py-6 text-center text-sm text-slate-500">No mail
                                        configuration has been added yet.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div id="mail-configuration-modal"
        class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/60 px-4 py-8">
        <div
            class="mx-auto w-full max-w-3xl overflow-hidden rounded-[32px] bg-white shadow-[0_30px_90px_-30px_rgba(15,23,42,0.35)]">
            <form id="mail-configuration-form" method="POST" action="{{ route('notification.configuration.store') }}"
                class="flex flex-col">
                @csrf
                ...
                <div class="border-b border-slate-200 px-6 py-6">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <h3 id="mail-modal-title" class="mt-2 text-2xl font-semibold text-slate-900">Create Mail
                                Configuration</h3>
                        ...
                        <button type="button" onclick="closeMailConfigurationModal()"
                            class="inline-flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200">
                ...
                <div class="space-y-6 px-6 py-6">
                    <div class="space-y-2">
                        ...
                        <div class="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-3">
                            <select id="state-select"
                                class="w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none focus:border-slate-400">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-3">
                            ...
                            <input id="from-email-input" type="email"
                                ...
                                onkeydown="handleEmailInput(event, 'from')" autocomplete="off" />
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-3">
                            ...
                            <input id="cc-email-input" type="email"
                                placeholder="Type email and press Enter, Comma, or Tab"
                                class="w-full bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-400"
                                onkeydown="handleEmailInput(event, 'cc')" autocomplete="off" />
                        </div>
                    </div>
