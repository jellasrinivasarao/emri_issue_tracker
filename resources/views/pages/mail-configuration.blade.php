<x-app-layout>
                        ...
                        <table class="min-w-full divide-y divide-slate-200">
                            <thead class="bg-purple-100 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-purple-900">Mail Type</th>
                                ...
                                @forelse($mailConfigurations ?? [] as $configuration)
                                    ...
                                    @endphp
                                        <tr>
                                            ...
                                            <td class="px-5 py-4 text-sm font-semibold uppercase text-slate-700">{{ ucfirst($configuration->recipient_type ?? ($configuration->state_id ? 'state' : 'ho')) }}</td>
                                            <td class="px-5 py-4 text-sm font-semibold text-slate-700">{{ $configuration->vendor?->vendor_name ?? ($configuration->recipient_type === 'vendor' ? 'Vendor #' . $configuration->vendor_id : '-') }}</td>
                                            ...
                                            <td class="px-5 py-4 text-sm"><span class="rounded-full {{ $configuration->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }} px-2.5 py-1 text-xs font-semibold">{{ $configuration->is_active ? 'Active' : 'Inactive' }}</span></td>
                                            <td class="px-5 py-4 text-sm text-slate-700">
                                                <div class="flex flex-wrap gap-2">
                                                    @if(data_get($permissions, 'edit'))
                                                        <button type="button" onclick="editMailConfiguration(@js(['id' => $configuration->mail_configuration_id, 'state_id' => $configuration->state_id, 'state_name' => $configuration->state_name, 'to_emails' => $toEmails, 'cc_emails' => $ccEmails]))" class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">Edit</button>
                                                    @endif
                                                    @if($configuration->is_active && data_get($permissions, 'deactivate'))
                                                        <form method="POST" action="{{ route('mail.configuration.toggle', $configuration->mail_configuration_id) }}" class="inline">@csrf<button type="submit" class="rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700 hover:bg-rose-100">Disable</button></form>
                                                    @elseif(!$configuration->is_active && data_get($permissions, 'activate'))
                                                        <form method="POST" action="{{ route('mail.configuration.toggle', $configuration->mail_configuration_id) }}" class="inline">@csrf<button type="submit" class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 hover:bg-emerald-100">Enable</button></form>
                                                    @endif
                                                </div>
                                            </td>
                                        </tr>
        ...
        <div
            ...
            <form id="mail-configuration-form" method="POST" action="{{ route('mail.configuration.store') }}"
                ...
                <div class="space-y-8 px-6 py-7 sm:px-10 sm:py-8">
                    ...
                    @else
                        <p class="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800">This rule will apply to all states.</p>
                    @endif
    ...
    function clearMailConfigForm() {
        ...
        const stateTriggerLabel = document.getElementById('state-trigger-label');
        if (stateTriggerLabel) stateTriggerLabel.textContent = 'Select state';
    ...
    async function editMailConfiguration(configuration) {
        ...
        const stateSelect = document.getElementById('state-select');
        if (stateSelect) {
            stateSelect.value = String(configuration.state_id || '');
    ...
    function saveMailConfiguration() {
        ...
        const stateLabel = stateValue ? (stateOption?.dataset.stateName || stateOption?.textContent || '') : 'All States';
