<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ $title ?? 'User–Support Group Mapping' }}</h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 bg-slate-50 px-5 py-5">
                    <p class="text-sm text-slate-600">{{ $description ?? 'Manage user membership in support groups.' }}</p>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-100">
                            <tr>
                                ...
                                <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-900">Support Group</th>
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                            @forelse($mappings as $mapping)
                                <tr>
                                    ...
                                    <td class="px-5 py-3 text-sm text-slate-600">{{ $mapping->support_group_name }}</td>
                                    <td class="px-5 py-3 text-sm">{{ $mapping->is_active ? 'Yes' : 'No' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-5 py-6 text-center text-sm text-slate-500">No user-support-group mappings found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
