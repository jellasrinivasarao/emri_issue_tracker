<x-app-layout>
    ...
    <div x-data="{ ...drawerState(), raiseIssueOpen: false, raiseIssueContent: '' }" x-init="init()">
        <div class="mx-auto max-w-full px-4 sm:px-6 lg:px-8">
            <!-- Raise Issue Button -->
            <div class="mb-4 flex justify-end">
                <button
                    @click="raiseIssueOpen = true; fetch('{{ route('issues.create.popup') }}').then(r => r.text()).then(html => { raiseIssueContent = html; setTimeout(() => { if (typeof window.initIssueCreatePopup === 'function') { window.initIssueCreatePopup(); } }, 0); })"
            ...
            <div class="grid gap-4 xl:grid-cols-[minmax(460px,1fr)_minmax(700px,1fr)]">
                ...
                <form id="roleIssueFilterForm" method="GET" action="{{ route('role.issue.dashboard') }}"
                    ...
                    <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-4">
                        <select name="state_id" onchange="this.form.submit()"
                            ...
                            <option value="{{ $state->state_id }}"
                                {{ (string) ($filterValues['state_id'] ?? '') === (string) $state->state_id ? 'selected' : '' }}>
                        ...
                        <select name="project_id" onchange="this.form.submit()"
                            ...
                            <option value="{{ $project->project_id }}"
                                {{ (string) ($filterValues['project_id'] ?? '') === (string) $project->project_id ? 'selected' : '' }}>
                        ...
                        <select name="application_id"
                            ...
                            <option value="{{ $application->application_id }}"
                                {{ (string) ($filterValues['application_id'] ?? '') === (string) $application->application_id ? 'selected' : '' }}>
                        ...
                        <select name="priority_id"
                            ...
                            <option value="{{ $priority->priority_id }}"
                                {{ (string) ($filterValues['priority_id'] ?? '') === (string) $priority->priority_id ? 'selected' : '' }}>
                                {{ $priority->priority_name }}</option>
                            @endforeach
                        </select>
                        <input type="date" name="date_from" value="{{ $filterValues['date_from'] ?? '' }}"
                            class="w-full rounded-[12px] border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-700 shadow-sm"
                            placeholder="From" />
                        <input type="date" name="date_to" value="{{ $filterValues['date_to'] ?? '' }}"
                            class="w-full rounded-[12px] border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-700 shadow-sm"
                            placeholder="To" />
                        <input type="text" name="ticket_id" value="{{ $filterValues['ticket_id'] ?? '' }}"
                            class="w-full rounded-[12px] border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 shadow-sm"
                    ...
                    <div class="mt-4 flex flex-wrap items-center gap-2">
                        <button type="submit"
                            class="rounded-[12px] bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Apply
                            Filters</button>
                        <a href="{{ route('role.issue.dashboard') }}"
                            class="rounded-[12px] border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                    </div>
                </form>
            </div>
            ...
            <div class="mt-4 rounded-[18px] border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div class="mt-3 max-h-[230px] overflow-y-auto rounded-[12px] border border-slate-200">
                    <table class="min-w-full divide-y divide-slate-200 text-left text-sm text-slate-700">
                        <thead
                            class="sticky top-0 z-10 bg-slate-50 text-[10px] uppercase tracking-[0.2em] text-slate-500">
                            <tr>
                        ...
                        <tbody class="divide-y divide-slate-200">
                            ...
                            <tr class="cursor-pointer hover:bg-slate-50"
                                @click="drawerOpen = true; selectedTicket = @js($ticket); selectedStatus = '{{ $ticket['status'] }}'; activeTab = 'details'; window.refreshVendorOptions?.(@js($ticket));">
                                <td class="px-3 py-2.5 font-semibold text-slate-900">
                                    <a href="#"
                                        @click.prevent="drawerOpen = true; selectedTicket = @js($ticket); selectedStatus = '{{ $ticket['status'] }}'; activeTab = 'details'; window.refreshVendorOptions?.(@js($ticket));"
                                        class="inline-block text-blue-600 hover:text-blue-800 underline decoration-blue-300 decoration-1 underline-offset-2">{{ $ticket['id'] }}</a>
                                </td>
                                ...
                                <td class="px-3 py-2.5">
                                    <span
                                        class="inline-flex rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-semibold text-emerald-700">{{ $ticket['status'] }}</span>
                                </td>
                                <td class="px-3 py-2.5">
                                    <span
                                        class="inline-flex rounded-full bg-red-100 px-2 py-1 text-[10px] font-semibold text-red-700">{{ $ticket['priority'] }}</span>
                                </td>
                ...
                fetch(updateForm.action, {
                    ...
                    .then(async (response) => {
                        ...
                        flashBox.innerHTML = message +
                            ' <button type="button" data-close-banner class="ml-2 rounded-full bg-white px-2 py-1 text-xs font-bold text-slate-700 hover:bg-slate-100">×</button>';
                        ...
                        if (closeButton) {
                            closeButton.closest('[x-data]')?.querySelector(
                                '[x-click="showUpdate = !showUpdate"]')?.click();
                        }
                    ...
                    .catch((error) => {
                        ...
                        flashBox.className =
                            'mb-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-800';
                        flashBox.innerHTML = message +
                            ' <button type="button" data-close-banner class="ml-2 rounded-full bg-white px-2 py-1 text-xs font-bold text-slate-700 hover:bg-slate-100">×</button>';
                        flashBox.classList.remove('hidden');
                ...
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <h3 class="mt-2 text-xl font-semibold text-slate-900"
                            x-text="selectedTicket ? selectedTicket.id : '—'">—</h3>
                        <div class="mt-2 flex flex-wrap items-center gap-2 text-sm">
                            <span class="rounded-full bg-slate-100 px-3 py-1 text-slate-700"
                                x-text="selectedTicket ? selectedTicket.status : '—'">—</span>
                            <span class="rounded-full bg-red-100 px-3 py-1 text-red-700"
                                x-text="selectedTicket ? selectedTicket.priority : '—'">—</span>
                        </div>
                    </div>
                    <button @click="drawerOpen = false; refreshIssueDashboard();"
                ...
                <div class="mt-6">
                    ...
                    <div class="mt-4 space-y-6">
                        <div x-show="activeTab==='details'" x-cloak class="space-y-4" x-data="{ showUpdate: false }">
                                ...
                                <div class="grid gap-3 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm font-semibold text-slate-900"
                                            x-text="selectedTicket ? selectedTicket.id : '—'">IT-xxxx</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] uppercase tracking-[0.24em] text-slate-500">State</p>
                                        <p class="mt-1 text-sm font-semibold text-slate-900"
                                            x-text="selectedTicket ? selectedTicket.state : '—'">State</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] uppercase tracking-[0.24em] text-slate-500">Project</p>
                                        <p class="mt-1 text-sm font-semibold text-slate-900"
                                            x-text="selectedTicket ? selectedTicket.project : '—'">Project</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] uppercase tracking-[0.24em] text-slate-500">Module</p>
                                        <p class="mt-1 text-sm font-semibold text-slate-900"
                                            x-text="selectedTicket ? selectedTicket.module : '—'">Module</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="rounded-[14px] border border-slate-200 bg-slate-50 p-4">
                                ...
                                <p class="mt-2 text-sm leading-6 text-slate-700"
                                    x-text="selectedTicket ? selectedTicket.description : 'No description available.'">
                                    No description available.</p>
                            </div>

                            <div class="flex items-center justify-between">
                                <p class="text-sm font-semibold text-slate-900">Update Ticket</p>
                                <button @click="showUpdate = !showUpdate" class="text-sm text-blue-600"> <span
                                        x-text="showUpdate ? 'Hide' : 'Edit'"></span></button>
                            </div>

                            <div x-show="showUpdate" x-cloak
                                class="rounded-[14px] border border-slate-200 bg-white p-4">
                                <form id="issueUpdateForm" method="POST" action="{{ route('role.issue.update') }}"
                                    enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" name="issue_id"
                                        x-bind:value="selectedTicket ? selectedTicket.issue_id : ''" />
                                    <input type="hidden" name="status_id"
                                        x-bind:value="selectedTicket ? selectedTicket.status_id : ''" />

                                    <div id="drawerUpdateMessage"
                                        class="hidden mb-3 rounded-xl border px-4 py-3 text-sm font-semibold"></div>

                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-sm font-semibold text-slate-700">Status</label>
                                            <select name="status_name" x-model="selectedStatus"
                                                class="mt-2 w-full rounded-[12px] border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                                @foreach($statusOptions as $status)
                                                <option value="{{ $status['status_name'] }}"
                                                    {{ ($status['status_name'] ?? '') === ($ticketStatus ?? '') ? 'selected' : '' }}>
                                                    {{ $status['status_name'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <div x-show="selectedStatus && ['escalate to vendor', 'vendor assignment'].includes(selectedStatus.toLowerCase())"
                                            x-cloak>
                                            <label class="block text-sm font-semibold text-slate-700">Vendor
                                                Assignment</label>
                                            <div id="vendor-multi-select" class="relative mt-2">
                                                <div class="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm transition focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100"
                                                    data-multi-select>
                                                    <div class="flex flex-wrap gap-2" data-multi-select-chips></div>
                                                    <input type="text"
                                                        class="min-w-[140px] flex-1 bg-transparent text-sm text-slate-900 placeholder:text-slate-400 outline-none"
                                                        placeholder="Search vendors" data-multi-select-input
                                                        autocomplete="off" />
                                                </div>
                                                <div class="absolute left-0 right-0 z-50 mt-1 hidden max-h-60 overflow-auto rounded-xl border border-slate-200 bg-white shadow-xl"
                                                    data-multi-select-list></div>
                                                <div data-multi-select-hidden class="hidden"></div>
                                            </div>
                                            <p class="mt-1 text-[11px] text-slate-500">Search and select one or more
                                                vendors.</p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <textarea name="remarks"
                                                class="mt-2 w-full rounded-[12px] border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm"
                                                rows="3" placeholder="Enter remarks..."></textarea>
                                        </div>

                                        <div>
                                            <label class="block text-sm font-semibold text-slate-700">Upload
                                                Attachment</label>
                                        ...
                                        <div class="flex justify-end pt-2">
                                            <button type="submit"
                                                class="rounded-[10px] bg-blue-600 px-4 py-2 text-sm font-semibold text-white">Save</button>
                                            <button type="button" @click="showUpdate = false"
                                                class="ml-2 rounded-[10px] border border-slate-200 px-4 py-2 text-sm">Cancel</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        ...
                        <div x-show="activeTab==='history'" x-cloak class="space-y-3">
                                ...
                                <template x-for="(row, index) in selectedTicket.history" :key="index">
                                    <div class="rounded-[16px] border border-slate-200 bg-white p-4">
                                        <div class="flex items-start justify-between gap-3">
                                            <div>
                                                <p class="text-sm font-semibold text-slate-900" x-text="row.changed_at">
                                                    —</p>
                                                <p class="mt-1 text-sm text-slate-500" x-text="row.changed_by">Update
                                                    user</p>
                                            </div>
                                            <span
                                                class="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-semibold text-slate-600"
                                                x-text="row.status_name">Open</span>
                                        </div>
                            ...
                            <template
                                x-if="selectedTicket && (!selectedTicket.history || selectedTicket.history.length === 0)">
                                <div
                                    class="rounded-[14px] border border-dashed border-slate-300 bg-white px-4 py-4 text-sm text-slate-500">
                                    No status history found.</div>
                            </template>
                        </div>

                        <div
                            class="sticky bottom-0 left-0 z-20 mt-4 rounded-[18px] border border-slate-200 bg-white p-4 shadow-xl">
                            <div class="flex justify-end">
                                <button @click="drawerOpen = false"
                                    class="rounded-[10px] border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Close</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    const vendorStateMappings = {{ Illuminate\Support\Js::from(
        ...
        function getAvailableVendorIds() {
            ...
            return vendorStateMappings
                .filter((mapping) => String(mapping.state_id) === String(currentStateId) && String(mapping
        ...
        function renderList(filter = '') {
            ...
            if (filtered.length === 0) {
                ...
                empty.textContent = currentStateId && currentProjectId ?
                    'No vendors mapped for this state/project.' : 'Select a ticket first to load vendors.';
        ...
        function renderChips() {
            ...
            selectedVendorIds.forEach((vendorId) => {
                ...
                removeButton.className =
                    'rounded-full bg-slate-200 px-1 text-slate-500 hover:bg-slate-300';
    ...
    <aside x-show="raiseIssueOpen" x-cloak
        ...
        <div x-html="raiseIssueContent" @submit.prevent="document.getElementById('issueCreateForm')?.submit()"></div>
