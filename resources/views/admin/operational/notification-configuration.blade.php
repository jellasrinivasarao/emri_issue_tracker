<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ $title ?? 'Notification Configuration' }}</h2>
    </x-slot>
                                        ...
                                        <td class="px-5 py-4 text-sm text-slate-700">
                                            <div class="flex flex-wrap gap-2">
                                                @if(data_get($permissions, 'edit'))
                                                    <button type="button" onclick="editMailConfiguration({{ json_encode($configuration) }})" class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">Edit</button>
                                                @endif
                                                @if($configuration->is_active && data_get($permissions, 'deactivate'))
                                                    <form method="POST" action="{{ url('/mail-configuration') }}/{{ $configuration->mail_configuration_id }}/toggle" class="inline">
                                                        @csrf
                                                        <button type="submit" class="rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700 hover:bg-rose-100">Deactivate</button>
                                                    </form>
