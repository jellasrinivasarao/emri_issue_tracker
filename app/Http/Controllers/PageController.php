            ...
            if (Schema::hasTable('txn_issue_attachment')) {
                ...
                $attachments = $attachmentRows->map(function ($row) {
                    return [
                        ...
                        'path' => $row->file_path ?: '',
                        'view_url' => rtrim(request()->getBaseUrl(), '/') . route('attachment.preview', ['id' => $row->attachment_id], false),
                        'inline_url' => rtrim(request()->getBaseUrl(), '/') . route('attachment.view', ['id' => $row->attachment_id], false),
                        'download_url' => rtrim(request()->getBaseUrl(), '/') . route('attachment.download', ['id' => $row->attachment_id], false),
